...
def sitAtual():
    r = requests.get('http://538167cf.ngrok.io/atoms')
    print r.json()

def insert(name, numatom, description):
    descricao = open(description)
    descricao = descricao.read()
    r = requests.post('http://538167cf.ngrok.io/atom', data={'element': name, 'numatom': numatom, 'description': descricao})
    print r.status_code
    print r.text
def delete(id):
    link = 'http://538167cf.ngrok.io/atom/' + id
...
def atualizar(id, name, numatom, description):
    ...
    r = requests.patch('http://538167cf.ngrok.io/atom/'+str(id), data={'id': id ,'element': name, 'numatom': numatom, 'description': descricao})
    print r.status_code
    print r.text
def main():
    print '''
